...
**Goal:** Make 8sync bg pick show a thumbnail image in the fzf preview pane using wezterm imgcat on the preview URL from the cache.
**Demo:** After this: # S02: bg pick Image Preview in fzf — UAT
...
## Tasks
- [x] **T01: Add wezterm imgcat thumbnail preview to 8sync bg pick: fzf now shows a right-pane thumbnail by downloading the preview URL to a temp file via Net.WebClient and rendering with wezterm imgcat --width 60, with fallback to page URL when wezterm is absent** — 
  - Files: modules/bg.ps1
  - Verify: pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ". .\wezterm-bootstrap.ps1; Read-BgCache | Select-Object -First 3"
