...
**Goal:** Rewrite Show-GpuStatus so it surfaces the actual GPU name, front_end (WebGpu/OpenGL), and power preference — not just a min_percent number.
**Demo:** After this: # S01: GPU Status Surface — UAT
...
## Tasks
- [x] **T01: Created modules/gpu.ps1 with enriched Show-GpuStatus (GPU name, front_end, power pref, min%) and new 8sync gpu verify PASS/FAIL subcommand** — 
  - Files: modules/gpu.ps1, modules/shell.ps1
  - Verify: pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ". .\wezterm-bootstrap.ps1; Invoke-GpuCommand -Rest @('status')"
