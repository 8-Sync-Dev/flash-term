...
function Show-8SyncHint {
    ...
    Write-HintRow '8sync jcode setup'                'Auto-install jcode + verify PATH + probe providers + smoke test'
    Write-HintRow '8sync jcode install [--force]'    'Install / update jcode (1jehuang/jcode)'
    Write-HintRow '8sync jcode status'               'Show jcode version + which AI providers are pre-configured'
    Write-HintRow '8sync jcode login <provider>'     'claude | copilot | openai | gemini | azure | fireworks | minimax | alibaba-coding-plan'
    Write-HintRow '8sync jcode browser status|setup' 'Check / set up jcode browser automation tool'
