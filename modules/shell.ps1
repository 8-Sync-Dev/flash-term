...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','forge','gsd','gsd-1','gguf','remove')
        ...
        $subMap = @{
            ...
            forge    = @('install','update','zsh','enter','zsh-enter','zsh-usage','usage','lightmode','light-mode','light','sync-to-gsd','sync','status','login','provider','uninstall','remove','help','--force','--dry-run','--with-zsh','--skip-omz','--name')
            gsd = @('setup','fix','key','keys','status','local','global','model','add','connect','remove','help','init','baseline','add-submodule','use','install','build','apply-anthropic-patch','enter','leave','latest','promote','rollback','forge-sync','--dry-run','--auto','--pick','--stable','--force','--refresh','--allow-global','--balance','--plan','--model','--tier','--only','--planning','--exec','--use-model','--ref','--version','--from','--backup','--skip-submodule','--skip-enter','--no-auto','--here','--project','opus+sonnet+haiku','opus+sonnet','sonnet+haiku','sonnet','light','balanced','heavy','max','pro','normal','claude-max','claude-code','claude-codex-review','codex-max','gemini-max','claude-codex-gemini','glm-max','claude','codex','gemini','glm','kimi','groq','copilot','gguf','--port','--name','zai','kimi-coding','groq','google','anthropic','openai','tavily','brave','ollama','context7','jina')
