...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','gsd','gguf')
        ...
        $subMap = @{
            ...
            gsd = @('setup','key','keys','status','add','connect','remove','help','--dry-run','--auto','--pick','--balance','--plan','--model','max','pro','normal','claude-max','codex-max','gemini-max','claude-codex-gemini','glm-max','claude','codex','gemini','glm','kimi','groq','copilot','gguf','--port','--name','zai','kimi-coding','groq','google','anthropic','openai','tavily','brave','ollama','context7','jina')
