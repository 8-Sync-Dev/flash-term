...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','gsd','gsd-1','gguf','remove')
        ...
        $subMap = @{
            ...
            opencode = @('export','apply','reinstall','fresh-install','deep-clean','uninstall-claude','install','setup','status','connect','help','cli','--cli','--dry-run','--force','--model','--plan','--claude=yes','--claude=max20','--claude=no','--openai=yes','--openai=no','--gemini=yes','--gemini=no','--copilot=yes','--copilot=no','claude-max','codex-max','gemini-max','glm-max','claude-codex-gemini','claude','codex','gemini','glm','groq','gguf')
