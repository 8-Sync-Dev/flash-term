...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','forge','gsd','gsd-1','gguf','remove')
        ...
        $subMap = @{
            ...
            gsd = @('setup','fix','key','keys','status','local','model','add','connect','remove','help','init','baseline','add-submodule','use','install','build','apply-anthropic-patch','enter','leave','latest','--dry-run','--auto','--pick','--stable','--force','--refresh','--allow-global','--balance','--plan','--model','--tier','--only','--planning','--exec','--use-model','--ref','--version','--from','--skip-submodule','--skip-enter','--no-auto','opus+sonnet+haiku','opus+sonnet','sonnet+haiku','sonnet','light','balanced','heavy','max','pro','normal','claude-max','claude-codex-review','codex-max','gemini-max','claude-codex-gemini','glm-max','claude','codex','gemini','glm','kimi','groq','copilot','gguf','--port','--name','zai','kimi-coding','groq','google','anthropic','openai','tavily','brave','ollama','context7','jina')
