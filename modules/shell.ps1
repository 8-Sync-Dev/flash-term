...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','gsd','gguf')
        ...
        $subMap = @{
            ...
            opencode = @('export','apply','reinstall','install','setup','status','connect','help','project','cli','--cli','--dry-run','--force','--model','--plan','claude-max','codex-max','gemini-max','glm-max','claude-codex-gemini','claude-codex-glm','claude','codex','gemini','glm','groq','gguf')
