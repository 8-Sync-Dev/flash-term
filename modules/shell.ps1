...
function Register-8SyncCompleter {
    ...
    $modes = @('help','status','reload','sync','clean','gpu','bg','hx','theme','opencode','gsd','gsd-1','gguf','remove')
        ...
        $subMap = @{
            ...
            gsd = @('setup','fix','key','keys','status','add','connect','remove','help','--dry-run','--auto','--pick','--stable','--force','--refresh','--balance','--plan','--model','--tier','--only','--planning','--exec','light','balanced','heavy','max','pro','normal','claude-max','claude-codex-review','codex-max','gemini-max','claude-codex-gemini','glm-max','claude','codex','gemini','glm','kimi','groq','copilot','gguf','--port','--name','zai','kimi-coding','groq','google','anthropic','openai','tavily','brave','ollama','context7','jina')
